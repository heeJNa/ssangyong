package chapter8.com.sist.jsoup;
...
public class MainClass {
	 ...
	 * Auto-generated method stub // 네트워크 (예외처리) try { // 정상적으로 실행되는 문장 Document
	 * doc=Jsoup.connect("https://www.genie.co.kr/chart/top200") .get();
	 * //System.out.println(doc.toString()); Elements title=doc.select("a.title");
	 * for(int i=0;i<title.size();i++) {
	 * System.out.println((i+1)+"."+title.get(i).text()); } }catch(Exception ex) {
	 * ex.printStackTrace();//에러 확인 } }
	 */
	/*
	 * catch => 복구가 가능 throws => 복구는 할 수 없고 에러 확인만 가능
