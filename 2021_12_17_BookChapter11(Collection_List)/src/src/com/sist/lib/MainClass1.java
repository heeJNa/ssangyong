...
public class MainClass1 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 홍길동 0 심청이 1 박문수 2 이순신 3 김두한 4
		 * 
		 * 
		 * 홍길동 0 심청이 1 강감찬 2 | 박문수 3 이순신 4 김두한 5
		 * 
		 * ==> 맨뒤에 추가 (속도상의 문제가 없다) ==> 중간에 추가 (속도가 느려진다)
		 * 
		 * 삭제 ==> 심청이 삭제 홍길동 0 강감찬 2 => 1 박문수 3 => 2 이순신 4 => 3 김두한 5 => 4
		 * 
		 * => 인덱스번호는 ArrayList자체에서 자동 처리 => 0번부터 순차적으로 유지 => 반복문을 수행하기 쉽게 => 인덱스번호 자동 조절
		 */
		...
		for (int i = 0; i < list.size(); i++) {
			...
			// 오른편이 크면 반드시 형변환을 한다
			/*
			 * 클래스의 형변환 (크기) A(동물) --- | --------- | | B(개) C(돼지) => 상속내린 클래스가 상속을 받을 클래스보다
			 * 데이터형이 크다 메모리 크기는 상속을 받은 클래스가 크다 => class A class B extends A class C extends
			 * B
			 * 
			 * => 비교 A a=new A(); B b=new B(); C c=new C();
			 * 
			 * ==> a=b (정상) === a가 b보다 크다 a=c (정상) === a가 c보다 크다 b=(B)a (오류) ==> 작은 클래스가 큰
			 * 클래스를 받았기 때문에 === c=(C)a b=c ==> 오류가 발생 (완전히 다른 클래스를 오류) === 크기를 비교할 수 없다
			 */
		}
		...
		/*
		 * 0.홍길동 1.춘향이 2.이순신 3.김두한 ==> 삭제 4.강감찬 5.박문수
		 * 
		 * ======= 삭제후 0.홍길동 1.춘향이 2.이순신 4=>3.강감찬 5=>4.박문수
		 */
		...
		/*
		 * ArrayList = 추가 (add()) = 갯수 (size()) = 수정 (set()) = 삭제 (remove()) = 읽기
		 * (get())
		 */
		// 배열이용
