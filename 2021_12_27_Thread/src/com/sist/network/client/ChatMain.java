package com.sist.network.client;
...
import com.sist.network.common.*;
...
public class ChatMain extends JFrame implements Runnable, ActionListener {
    ...
    CardLayout card = new CardLayout(); //하나의 윈도우에 여러 화면을 번갈아가며 출력
    // 화면 UI
    // 네트워크와 관련된 클래스
    // 연결 도구
    Socket s;
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == login.b2) {
            ...
            System.exit(0); //종료해라
        } else if (e.getSource() == login.b1) {  //로그인 버튼을 클릭하면
            // 1. 입력한 값을 읽는다
            // 1-1 ID읽기
            String id = login.tf1.getText(); // 입력한 값 읽어오기
            // 유효성 검사
