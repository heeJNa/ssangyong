package com.sist.network.server;
// ==> Server : 사용자 요청 => 처리 => 결과값을 전송한다
// ==> 접속시 처리 / 사용자마다 통신 (Thread)

import com.sist.network.common.Function;
...
public class Server implements Runnable {
    ...
    private ArrayList<Client> waitVc = new ArrayList<>();
    ...
    @Override
    public void run() {
        while (true) { // 서버가 종료하기 전까지 접속을 받는다
            try {
    ...
    class Client extends Thread {
        ...
        public Client(Socket s) {
            ...
            try {
                ...
                /*
                 *       char => 1byte (싱글바이트) => ASCII(아스키)
                 *               0~256 (C/C++)
                 *       char => 2byte (멀티바이트) => Unicode
                 *               0~65535 (Java,C#,Python)
                 *       *** ASC vs Unicode
                 * */
            } catch (Exception e) {
                e.printStackTrace();
        ...
        public void messageTo(String msg) {  //비동기화
            try {
                ...
                // decoding => InputStreamReader => 필터 스트림
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
